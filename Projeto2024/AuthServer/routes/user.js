...
router.post('/register', auth.verificaAdmin,  function (req, res) {
    ...
    userModel.register(new userModel({
            username: req.body.username, name: req.body.name, email: req.body.email, ucs: req.body.ucs,
...
router.post('/registerAdmin', auth.verificaAdmin,  function (req, res) {
    ...
    userModel.register(new userModel({
            ...
            level: "admin", dateCreated: d.substring(0, 10), lastAccess: d.substring(0, 19), ucs: []
        }),
        req.body.password,
        function (err, user) {
            if (err)
...
router.post('/registerDocente', auth.verificaAdmin,  function (req, res) {
    ...
    userModel.register(new userModel({
            username: req.body.username, name: req.body.name, email: req.body.email,
            level: "prod", dateCreated: d.substring(0, 10), lastAccess: d.substring(0, 19), ucs: []
        }),
        req.body.password,
        function (err, user) {
            if (err)
...
router.post('/login', passport.authenticate('local'), function (req, res) {
    jwt.sign({
            _id: req.user._id, email: req.user.email, level: req.user.level, ucs: req.user.ucs
        },
        "EngWeb2024",
